...
class Puzzle1Spec extends ObjectBehavior
{
    ...
    public function it_should_parse_instructions() {
        $this->parseInstruction('rect 4x1')->shouldReturn(['rect', 4, 1]);
        $this->parseInstruction('rotate column x=10 by 1')->shouldReturn(['rotate', 'Column', 10, 1]);
        $this->parseInstruction('rotate row y=2 by 16')->shouldReturn(['rotate', 'Row', 2, 16]);
    }

    public function it_should_come_up_with_an_answer() {
        $input = file_get_contents(__DIR__ . '/../../src/Day08/input.txt');
        $this->__invoke($input)->shouldNotEqual(111); // too low
    }
}
